// app/api/admin/lookups/funds/validate/route.ts
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: currentLookups, error: fetchError } = await supabaseAdmin
      .from('funds_dim')
      .select('id, fund_code, fund_name, effective_start_fy, effective_end_fy')
    ...
    const mappedCurrent = (currentLookups ?? []).map((row: FundDimRow) => ({
      id: row.id,
      code: row.fund_code,
      name: row.fund_name,
    ...
    console.error('Funds validate error:', error);
    return NextResponse.json(
