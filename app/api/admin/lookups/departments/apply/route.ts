// app/api/admin/lookups/departments/apply/route.ts
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Check lookup type
    if (validated.lookupType !== 'departments') {
      return NextResponse.json(
    ...
    if (changes.toCloseOut.length > 0) {
      ...
      const { data: currentEntries } = await supabaseAdmin
        ...
        .select('*')
        .in('department_code', closeOutCodes)
      ...
      const closeLogs = (currentEntries ?? []).map(entry => ({
        ...
        action: 'close',
        lookup_code: entry.department_code,
        old_values: entry,
    ...
    if (changes.toInsert.length > 0) {
      ...
      const insertLogs = changes.toInsert.map(item => ({
        ...
        old_values: null,
        new_values: { department_code: item.code, department_name: item.name, effective_start_fy: item.effectiveStartFy },
    ...
    const { error: refreshError } = await supabaseAdmin.rpc('refresh_departments_by_year');
    ...
    console.error('Departments apply error:', error);
    return NextResponse.json(
