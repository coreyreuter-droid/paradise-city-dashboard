...
export default function BudgetClient({ years, deptBudgetActuals }: Props) {
  ...
  const totals = useMemo(() => {
    ...
    const execPct = budget > 0 ? Math.min((actualsSum / budget) * 100, 999) : 0;
    ...
    () => [
      {
        ...
        cell: (row: DepartmentSummary) => {
          ...
          return (
            <Link
              href={`${cityHref(`/departments/${encodeURIComponent(name)}`)}?year=${chartYear}`}
      ...
      },
      {
        ...
        header: "Budget",
        align: "right",
        cell: (row: DepartmentSummary) => <span>{formatCurrency(row.budget)}</span>,
      },
      {
        key: "actuals",
        header: "Actuals",
        align: "right",
        cell: (row: DepartmentSummary) => <span>{formatCurrency(row.actuals)}</span>,
      },
      {
        key: "percentSpent",
        header: "% spent",
        align: "right",
        cell: (row: DepartmentSummary) => <span>{formatPercent(row.percentSpent, 1)}</span>,
      },
      {
        key: "variance",
        header: "Variance",
        ...
        cell: (row: DepartmentSummary) => {
          ...
          const color = v > 0 ? " text-emerald-700" : v < 0 ? " text-red-700" : " text-slate-700";
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      <div className="mx-auto max-w-6xl px-4 py-8">
        ...
        <nav aria-label="Breadcrumb" className="mb-4 px-1 text-xs text-slate-500">
        ...
        <div className="space-y-6">
          <CardContainer>
            <div className="space-y-6">
                ...
                <div className="text-xs text-slate-500">
                  {yearLabel ? (
                    <>
                      ...
                      {!hasAnyActualsForSelectedYear && " Actuals have not been uploaded for this year yet."}
                    </>
                  ) : (
              ...
              <section aria-label="Budget execution summary" className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Total budget</div>
                  <div className="mt-1 text-lg font-semibold text-slate-900">{formatCurrency(totals.budget)}</div>
                  <div className="mt-1 text-xs text-slate-500">Sum of department-level adopted budgets.</div>
                </div>

                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">Total actuals</div>
                  <div className="mt-1 text-lg font-semibold text-slate-900">{formatCurrency(totals.actuals)}</div>
                  <div className="mt-1 text-xs text-slate-500">All spending recorded against these departments.</div>
                </div>
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3">
                  <div className="text-xs font-semibold uppercase tracking-wide text-slate-500">% of budget spent</div>
                  <div className="mt-1 text-lg font-semibold text-slate-900">{formatPercent(totals.execPct, 1)}</div>
                  <div className="mt-1 text-xs text-slate-500">
          ...
          <CardContainer>
            {departments.length === 0 ? (
              <p className="text-sm text-slate-500">No budget or actuals data available for this year.</p>
            ) : (
              <div className="space-y-6">
                <section
                    ...
                    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                      ...
                      <div className="mt-3 h-2 w-full overflow-hidden rounded-full bg-slate-200" aria-hidden="true">
                        <div
                          ...
                          style={{ width: `${Math.min(totals.execPct, 100)}%` }}
                        />
                      </div>
                    </div>
                  </div>
                ...
                <BudgetByDepartmentChart year={chartYear} departments={departments} showTable={false} />
